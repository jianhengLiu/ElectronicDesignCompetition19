...
void SendMotor()
{
	if(Fly.WorkState == STOP)        //如果机器人为停止状态，则电机初始化为0
	{
		CanTransmit(&hcan,0);
	}
	else                                  //当机器人不是停止状态时
	{
		CanTransmit(&hcan,Fly.Motor.output);
	}
}
...
* @retval None
*/
void CanTransmit(CAN_HandleTypeDef *hcanx, int16_t cm1_iq)
{
...
void CanDataReceive(int motor_index)
{
		...
		if(motor_index == CAN_CHASSIS_MOTOR1_ID)
			{
			if(Fly.Motor.msg_cnt++ < 50 ){
				get_rotor_offset(&Fly.Motor);
			}
			else{
			CanDataDecoder(&Fly.Motor);    //电机数据具体解析函数
			}
		}
